    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                ...
                color: paid ? const Color(0xFF157548) : const Color(0xFF8A6118),
              ),
          ...
          if (order.invoiceNumber?.isNotEmpty == true)
            Padding(
          ...
          if (order.payment?.merchantNote?.trim().isNotEmpty == true)
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Text(
                order.payment!.merchantNote!,
                style: const TextStyle(height: 1.5),
              ),
            ),
        ],
...
final class _ManualPaymentPageState extends State<ManualPaymentPage> {
  ...
  Future<void> _submit() async {
    if (_paymentNote.text.trim().length < 2 &&
        _reference.text.trim().length < 2 &&
        _receipt == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('اكتب إشعار الدفع أو أرفق صورة الإيصال.')),
      );
      return;
    }
    setState(() => _submitting = true);
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        const SizedBox(height: 14),
        if (widget.order.manualPayment?.depositAddress?.trim().isNotEmpty ==
            true)
          _ManualPaymentInstructionsCard(
            instructions: widget.order.manualPayment!,
          ),
        ...
        TextField(
          ...
          maxLines: 4,
          decoration: const InputDecoration(
            labelText: 'إشعار الدفع',
            hintText:
                'اكتب اسم المحوّل أو أي تفاصيل تساعد التاجر على المطابقة.',
