...
final class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void initState() {
    super.initState();
    if (!widget.cart.acceptsDelivery && widget.cart.acceptsPickup)
      _fulfillment = 'pickup';
  }
        ...
        return ListView(
          ...
          children: [
            ...
            SegmentedButton<String>(
              segments: [
                if (widget.cart.acceptsDelivery)
                  const ButtonSegment(
                    value: 'delivery',
                    icon: Icon(Icons.local_shipping_outlined),
                    label: Text('توصيل'),
                  ),
                if (widget.cart.acceptsPickup)
                  const ButtonSegment(
                    value: 'pickup',
                  ...
                  children: [
                    ...
                    if (widget.cart.manualPayment?.enabled == true) ...[
                      ...
                      const RadioListTile<String>(
                        ...
                        subtitle: Text(
                          'ستظهر تعليمات الإيداع ثم ترسل إشعاراً كتابياً أو صورة إيصال.',
                        ),
                      ),
                    ],
                  ],
            ...
            if (_paymentMethod == 'manual_transfer' &&
                widget.cart.manualPayment != null) ...[
              const SizedBox(height: 10),
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(
                ...
                color: Color(0xFF8A6118),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  instructions.depositTitle?.trim().isNotEmpty == true
                      ? instructions.depositTitle!
                      : 'عنوان الإيداع',
          ...
          if (instructions.instructions?.trim().isNotEmpty == true) ...[
            const SizedBox(height: 8),
            Text(
    ...
    builder: (context, snapshot) {
      if (snapshot.connectionState != ConnectionState.done) {
        return const Card(
          child: ListTile(
            ...
            title: Text('جارٍ حساب مسافة الطريق ورسوم التوصيل…'),
          ),
        );
      }
      if (snapshot.hasError) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            title: const Text('لا يمكن تأكيد توصيل هذا العنوان'),
      ...
      return Card(
        ...
        child: ListTile(
          ...
          title: Text(
            'التوصيل ${quote.distanceKm.toStringAsFixed(1)} كم · نحو $minutes دقيقة',
          ),
          subtitle: Text(
            'الحد الأدنى ${quote.baseFee.toStringAsFixed(2)} + ${quote.perKmFee.toStringAsFixed(2)} لكل كم',
          ),
          trailing: Text(
            '${quote.deliveryFee.toStringAsFixed(2)} ${quote.currencyCode}',
            style: const TextStyle(fontWeight: FontWeight.w900),
          ),
        ),
      );
