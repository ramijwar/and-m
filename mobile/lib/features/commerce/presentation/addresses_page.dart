...
final class _AddressesPageState extends State<AddressesPage> {
  ...
  void _reload() =>
      setState(() => _future = AppScope.of(context).loadAddresses());
...
final class _AddressFormPageState extends State<AddressFormPage> {
  ...
  void initState() {
    ...
    _selectedPoint = latitude != null && longitude != null
  ...
  Future<void> _applyMapPoint(LatLng point) async {
    ...
    try {
      ...
      if (!mounted || version != _lookupVersion) return;
      setState(
        () => _locationMessage = 'تم تحديد الموقع، لكن تعذر تعبئة العنوان تلقائياً. أكمل الحقول يدوياً.',
      );
    } finally {
  ...
  Future<void> _save() async {
    ...
    if (_selectedPoint == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('حدد موقع التوصيل على الخريطة أو استخدم GPS أولاً.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      await AppScope.of(context).saveAddress(
        CustomerAddress(
          ...
          label: 'عنوان التوصيل',
          recipientName:
              session?.fullName ?? widget.address?.recipientName ?? '',
          phone:
              session?.user['phone'] as String? ?? widget.address?.phone ?? '',
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(
        widget.address == null ? 'إعداد عنوان التوصيل' : 'تعديل عنوان التوصيل',
      ),
    ),
    body: Form(
      key: _formKey,
