...
final class _HomeShellState extends State<HomeShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: controller.session == null
                ...
                : () => Navigator.of(context).push(
                    MaterialPageRoute<void>(
                      builder: (_) =>
                          const MarketplaceActivityPage(initialIndex: 2),
                    ),
                  ),
          ...
          IconButton(
            ...
            onPressed: controller.session == null
                ...
                : () => Navigator.of(context).push(
                    MaterialPageRoute<void>(
                      builder: (_) =>
                          const MarketplaceActivityPage(initialIndex: 1),
                    ),
                  ),
      ...
      body: SafeArea(
        child: Column(children: [Expanded(child: pages[_tabIndex])]),
      ),
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _StoreModeFilters(mode: widget.mode, onChanged: widget.onModeChanged),
          const SizedBox(height: 16),
          if (controller.isStoreDirectoryLoading && !isCurrentMode)
            ...
            const FeaturedProductsPanel(
              ...
              emptyText:
                  'ستظهر المنتجات الأعلى تقييماً بعد تسجيل تقييمات العملاء.',
            ),
          ],
        ],
      ),
...
final class _MarketplaceTabState extends State<_MarketplaceTab> {
  ...
  Future<CatalogPage<MarketplaceListing>> _load() =>
      AppScope.of(context).browseMarketplacePage();
  ...
  ) => FutureBuilder<CatalogPage<MarketplaceListing>>(
    future: _future,
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'تُحدّث باستمرار من الحراج المحلي',
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text(
          'ابحث، تواصل، واعثر على ما يناسبك بالقرب منك.',
          style: TextStyle(
            color: Colors.white,
            height: 1.45,
            fontSize: 16,
          ...
          children: [
            Expanded(
              child: FilledButton.icon(
                style: FilledButton.styleFrom(
                  backgroundColor: Colors.white,
                  foregroundColor: const Color(0xFF155644),
...
final class _MarketplaceTabStateCard extends StatelessWidget {
  const _MarketplaceTabStateCard({
    required this.icon,
    required this.title,
    required this.body,
      ...
      children: [
        ...
        ...roles
            ...
            .map(
              (role) => _AccountAction(
                ...
                onTap: () => Navigator.of(context).push(
                  MaterialPageRoute<void>(
                    builder: (_) => WalletPage(role: role),
                  ),
                ),
              ),
            ),
        if (roles.contains('merchant'))
          _AccountAction(
            ...
            onTap: () => Navigator.of(context).push(
              MaterialPageRoute<void>(
                builder: (_) => const MerchantWorkspacePage(),
              ),
            ),
          ),
        if (roles.contains('merchant'))
          _AccountAction(
            icon: Icons.receipt_long_outlined,
            title: 'طلبات المتجر',
            subtitle: 'قبول الطلبات وتجهيزها وإتاحتها للتوصيل',
            onTap: () => Navigator.of(context).push(
              MaterialPageRoute<void>(
        ...
        if (roles.contains('merchant'))
          _AccountAction(
            ...
            onTap: () => Navigator.of(context).push(
              MaterialPageRoute<void>(
                builder: (_) => const MerchantProductsPage(),
              ),
            ),
          ),
...
final class _VerificationAccountAction extends StatefulWidget {
  ...
  @override
  State<_VerificationAccountAction> createState() =>
      _VerificationAccountActionState();
}

final class _VerificationAccountActionState
    extends State<_VerificationAccountAction> {
  ...
  String _label(List<Map<String, dynamic>> items) {
    final current =
        items
            .where((item) => item['role_code'] == widget.roleCode)
            .map(
              (item) =>
                  item['verification_status'] as String? ?? 'not_submitted',
            )
            .cast<String?>()
            .firstWhere(
              (value) => value != null,
  ...
  IconData _icon(String label) => switch (label) {
    'موثق' => Icons.verified_rounded,
    'مراجعة' => Icons.hourglass_top_rounded,
    _ => Icons.verified_user_outlined,
  };
  Future<void> _open() async {
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => AccountVerificationPage(roleCode: widget.roleCode),
      ),
    );
    if (mounted)
      setState(
  ...
  Widget build(BuildContext context) =>
      FutureBuilder<List<Map<String, dynamic>>>(
        future: _future,
        builder: (context, snapshot) {
          final label = _label(snapshot.data ?? const []);
    ...
    child: ListTile(
      ...
      subtitle: Column(
        ...
        children: [
          ...
          if (!compact &&
              (store.isWeeklyMostVisited || store.isBestSeller)) ...[
