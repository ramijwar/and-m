...
final class AdminNotificationCampaignPage extends StatefulWidget {
  ...
  @override
  State<AdminNotificationCampaignPage> createState() =>
      _AdminNotificationCampaignPageState();
}

final class _AdminNotificationCampaignPageState
    extends State<AdminNotificationCampaignPage> {
  ...
  Map<String, dynamic> get _payload => {
    ...
    'role_codes': _mode == 'roles'
        ? _roles.toList(growable: false)
        : const <String>[],
    'user_ids': _mode == 'users'
        ? _users.keys.toList(growable: false)
        : const <String>[],
    'target_type': _targetType,
    'target_entity_public_id': _targetType == 'none'
        ? ''
        : (_target?['entity_public_id'] as String? ?? ''),
  };

  void _refreshPreview() {
    if ((_mode == 'roles' && _roles.isEmpty) ||
        (_mode == 'users' && _users.isEmpty)) {
      setState(() => _preview = null);
      return;
    }
    setState(
      () =>
          _preview = AppScope.of(context)
              .previewAdminNotificationCampaign(_payload),
    );
  }
  ...
  String _audience(Map<String, dynamic> campaign) {
    if (campaign['audience_mode'] == 'all')
      return 'كل الحسابات النشطة غير الإدارية';
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (sheetContext, modalSet) {
          void search() {
            ...
            modalSet(
              () => found = AppScope.of(context).searchAdminUsers(search: term),
            );
          }
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'الاسم أو اسم المستخدم أو الهاتف',
                    ...
                    Expanded(
                      child: found == null
                          ? const Center(
                              child: Text('ابحث ثم اختر حساباً أو أكثر.'),
                            )
                          : FutureBuilder<List<Map<String, dynamic>>>(
                              ...
                              builder: (_, snap) {
                                ...
                                if (snap.hasError)
                                  return const Center(
                                    child: Text('تعذر البحث عن الحسابات.'),
                                  );
                                final rows = snap.data ?? const [];
                                if (rows.isEmpty)
                                  return const Center(
                                    child: Text('لا توجد حسابات نشطة مطابقة.'),
                                  );
  ...
  Future<Map<String, dynamic>?> _searchDestination({
    ...
    context: context,
    isScrollControlled: true,
    builder: (_) => _CampaignTargetSearchSheet(
      type: type,
      title: title,
      hint: hint,
      storeId: storeId,
  ...
  Future<void> _send() async {
    ...
    if (_mode == 'roles' && _roles.isEmpty) {
      _message('اختر دوراً واحداً على الأقل.');
      return;
    }
    if (_mode == 'users' && _users.isEmpty) {
      _message('اختر مستخدماً واحداً على الأقل.');
      return;
    }
    if (_targetType != 'none' &&
        (_target?['entity_public_id'] as String? ?? '').isEmpty) {
    ...
    try {
      final result = await AppScope.of(context)
          .createAdminNotificationCampaign(_payload);
      _message(
        result['replayed'] == true
            ? 'الحملة مسجلة مسبقاً.'
            : 'تم إنشاء الحملة ووضع ${result['campaign']?['queued_notification_count'] ?? 0} إشعار في الطابور.',
      );
      setState(() {
        _title.clear();
        _body.clear();
  ...
  Widget _destinationPicker() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const SizedBox(height: 12),
      DropdownButtonFormField<String>(
        ...
        decoration: const InputDecoration(
          labelText: 'الوجهة عند الضغط (اختيارية)',
        ),
        items: const [
  ...
  Widget build(BuildContext context) => AdminWorkspaceScaffold(
    active: AdminDestination.campaigns,
    appBar: AppBar(title: const Text('الحملات والإشعارات')),
    body: Form(
      key: _form,
      child: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'يحفظ الإشعار أولاً ثم يرسله Cron تدريجياً. لا تظهر بيانات الأجهزة هنا.',
                  ),
                  ...
                  TextFormField(
                    ...
                    validator: (value) => (value ?? '').trim().length < 2
                        ? 'اكتب عنواناً من حرفين على الأقل.'
                        : null,
                  ),
                  TextFormField(
                    controller: _body,
                  ...
                  RadioListTile<String>(
                    ...
                    title: const Text('كل الحسابات النشطة غير الإدارية'),
                    onChanged: (value) {
                  ...
                  if (_mode == 'roles')
                    Wrap(
                      spacing: 8,
                      children: ['customer', 'merchant', 'courier']
                          .map(
                            (role) => FilterChip(
                              label: Text(_role(role)),
                  ...
                  RadioListTile<String>(
                    ...
                    groupValue: _mode,
                    contentPadding: EdgeInsets.zero,
                    title: const Text('مستخدمون محددون'),
                    onChanged: (value) {
                      setState(() => _mode = value!);
                      _refreshPreview();
                    },
                    ...
                    Column(
                      ...
                      children: [
                        OutlinedButton.icon(
                          ...
                          label: const Text('بحث واختيار مستخدمين'),
                        ),
                        if (_users.isNotEmpty)
                          Wrap(
                            spacing: 6,
                            runSpacing: 6,
                            children: _users.values
                                .map(
                                  (user) => InputChip(
                                    label: Text(
                  ...
                  SwitchListTile.adaptive(
                    ...
                    onChanged: _sending
                        ? null
                        : (value) {
                            setState(() => _includeAdmins = value);
                            _refreshPreview();
                          },
                    title: const Text('إرسال نسخة إلى المديرين'),
                    subtitle: const Text(
                      'مفعّلة افتراضياً. تحفظ النسخة في صندوق كل مدير نشط ليطلع عليها.',
                    ),
                  ),
                  const SizedBox(height: 12),
                  _preview == null
                      ? const Text(
                          'سيظهر عدد المستلمين بعد اختيار جمهور صالح.',
                      ...
                      : FutureBuilder<Map<String, dynamic>>(
                          ...
                          builder: (_, snapshot) {
                            ...
                            if (snapshot.hasError)
                              return const Text('تعذر حساب العدد حالياً.');
                            final data = snapshot.data ?? const {};
                            final adminCopies =
                                data['admin_copy_count'] as num? ?? 0;
                            return Text(
                              'المطابقون: ${data['selected_user_count'] ?? 0} · المؤهلون وفق إعداداتهم: ${data['eligible_user_count'] ?? 0}${_includeAdmins ? ' · نسخة إدارية: $adminCopies' : ''}',
                  ...
                  FilledButton.icon(
                    ...
                    label: Text(
                      _sending
                          ? 'جارٍ إنشاء الحملة…'
                          : 'إنشاء الحملة ووضعها في الطابور',
                    ),
                  ),
                ],
              ),
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (_, snapshot) {
              ...
              if (snapshot.hasError)
                return const Card(
                  child: ListTile(title: Text('تعذر تحميل سجل الحملات.')),
                );
              final rows = snapshot.data ?? const [];
              if (rows.isEmpty)
                return const Card(
                  child: ListTile(title: Text('لا توجد حملات مسجلة.')),
                );
              return Column(
                children: rows
                    .map(
                      (campaign) => Card(
                        child: ListTile(
                          ...
                          title: Text(campaign['title'] as String? ?? 'إشعار'),
                          subtitle: Text(
                            '${_audience(campaign)}\n${_historyDestination(campaign)}\n${campaign['created_at'] ?? ''} · بواسطة ${campaign['sender_name'] ?? 'مدير'}',
                          ),
                          isThreeLine: true,
                          trailing: Text(
                            '${campaign['queued_notification_count'] ?? 0}\nفي الطابور',
...
final class _CampaignTargetSearchSheet extends StatefulWidget {
  const _CampaignTargetSearchSheet({
    required this.type,
    required this.title,
    required this.hint,
...
final class _CampaignTargetSearchSheetState
    extends State<_CampaignTargetSearchSheet> {
