...
AdminControlStrip _adminStrip(BuildContext context, String active) =>
    AdminControlStrip(
      active: active,
      onDashboard: () =>
          openAdminDestination(context, AdminDestination.dashboard),
      onUsers: () => openAdminDestination(context, AdminDestination.users),
      onQueues: () => openAdminDestination(context, AdminDestination.queues),
      onCommerce: () =>
          openAdminDestination(context, AdminDestination.commerce),
      onCatalog: () => openAdminDestination(context, AdminDestination.catalog),
      onMarketplace: () =>
          openAdminDestination(context, AdminDestination.marketplace),
      onSupport: () => openAdminDestination(context, AdminDestination.support),
      onDelivery: () =>
          openAdminDestination(context, AdminDestination.delivery),
      onFinance: () => openAdminDestination(context, AdminDestination.finance),
      onActivity: () =>
          openAdminDestination(context, AdminDestination.activity),
      onOperations: () =>
          openAdminDestination(context, AdminDestination.operations),
    );
...
PreferredSizeWidget adminTabbedBottom(
  ...
  preferredSize: Size.fromHeight(62 + tabs.preferredSize.height),
  child: Column(
