...
final class _AdminDeliveryPageState extends State<AdminDeliveryPage> {
  ...
  Future<void> _showCourier(String id) async {
    try {
      ...
      final courier = Map<String, dynamic>.from(
        data['courier'] as Map? ?? const {},
      );
      final tasks = (data['tasks'] as List? ?? const [])
          .whereType<Map>()
          .map((item) => Map<String, dynamic>.from(item))
          .toList(growable: false);
      final verification = data['latest_verification'] is Map
          ? Map<String, dynamic>.from(data['latest_verification'] as Map)
          : null;
        ...
        builder: (context) => _DetailSheet(
          ...
          sections: [
            _detailSection('بيانات الحساب', courier, const {
              ...
              'active_task_count': 'المهام النشطة',
              'last_task_at': 'آخر مهمة',
              'created_at': 'تاريخ الانضمام',
              'last_login_at': 'آخر دخول',
            }),
            if (verification != null)
              _detailSection('آخر طلب توثيق', verification, const {
                'public_id': 'المعرف',
        ...
        builder: (context) => _DetailSheet(
          ...
          sections: [
            _detailSection('المهمة والصفقة', task, const {
              'public_id': 'معرف المهمة',
              'source_type': 'مصدر المهمة',
              'source_label': 'المرجع',
              'order_public_id': 'معرف الطلب',
              'order_number': 'رقم الطلب',
              'transaction_public_id': 'معرف صفقة الحراج',
              'transaction_status': 'حالة الصفقة',
              'task_status': 'حالة المهمة',
              'courier_name': 'عامل التوصيل',
              'courier_phone': 'هاتف العامل',
              'customer_name': 'العميل/المشتري',
              'seller_name': 'البائع/التاجر',
              'store_name': 'المتجر',
              'delivery_fee_amount': 'رسم التوصيل',
              'courier_earning_amount': 'استحقاق العامل',
              'currency_code': 'العملة',
              'delivery_distance_km': 'المسافة كم',
              'delivery_base_fee_amount': 'الرسم الأساسي',
              'delivery_per_km_fee_amount': 'رسم كل كم',
            ...
            _detailSection(
              'عنوان الاستلام',
              task['pickup_address'] is Map
                  ? Map<String, dynamic>.from(task['pickup_address'] as Map)
                  : const {},
              const {},
            ),
            _detailSection(
              'عنوان التسليم',
              task['delivery_address'] is Map
                  ? Map<String, dynamic>.from(task['delivery_address'] as Map)
                  : const {},
              const {},
            ),
            if (task['pickup_proof_media_public_id'] != null ||
                task['delivery_proof_media_public_id'] != null)
              _proofSection(task),
  ...
  Future<void> _showProof(String taskId, String proofType) async {
    try {
      final Uint8List bytes = await AppScope.of(context)
          .loadAdminDeliveryTaskProof(taskId: taskId, proofType: proofType);
      if (!mounted) return;
      await showDialog<void>(
        context: context,
        builder: (context) => AlertDialog(
          title: Text(
            proofType == 'pickup' ? 'إثبات استلام الطلب' : 'إثبات تسليم الطلب',
          ),
  ...
  Widget build(BuildContext context) => DefaultTabController(
    ...
    child: AdminWorkspaceScaffold(
      active: AdminDestination.delivery,
      appBar: AppBar(
        ...
        bottom: adminTabbedBottom(
          ...
          const TabBar(
            tabs: [
              ...
              Tab(icon: Icon(Icons.route_outlined), text: 'المهام'),
            ],
          ),
        ),
      ),
  ...
  Widget _couriersTab() => _SearchList(
    ...
    matches: (item, text) =>
        '${item['full_name'] ?? ''} ${item['username'] ?? ''} ${item['phone'] ?? ''} ${item['verification_status'] ?? ''} ${item['work_status'] ?? ''}'
            .toLowerCase()
            .contains(text),
    filters: const {
      'verification_status': {
        'not_submitted': 'دون توثيق',
    ...
    builder: (item) {
      ...
      return Card(
        child: ListTile(
          ...
          title: Text(
            item['full_name'] as String? ?? 'عامل توصيل',
                  ...
                  onSelected: (value) {
                    if (value.startsWith('verify:'))
                      _run(
                        'courier-$id',
                        () => AppScope.of(context)
                            .updateAdminCourierVerification(
                              courierId: id,
                              status: value.substring(7),
                            ),
                        'تم تحديث توثيق العامل.',
                      );
                    if (value.startsWith('work:'))
                      _run(
                        'courier-$id',
                        () => AppScope.of(context).updateAdminCourierWorkStatus(
                          courierId: id,
                          status: value.substring(5),
                        ),
                        'تم تحديث حالة عمل العامل.',
                      );
                  },
                  itemBuilder: (_) => const [
                    PopupMenuItem(
                      value: 'verify:verified',
                      child: Text('اعتماد التوثيق'),
                    ),
                    PopupMenuItem(
                      value: 'verify:pending',
                      child: Text('إعادة التوثيق إلى المراجعة'),
                    ),
                    PopupMenuItem(
                      value: 'verify:rejected',
                      child: Text('رفض التوثيق'),
                    ),
                    PopupMenuDivider(),
                    PopupMenuItem(
                      value: 'work:available',
                      child: Text('جعله متاحاً'),
                    ),
                    PopupMenuItem(
                      value: 'work:offline',
                      child: Text('جعله غير متصل'),
                    ),
                    PopupMenuItem(
                      value: 'work:suspended',
                      child: Text('تعليق عمله'),
                    ),
                  ],
                ),
        ),
  ...
  Widget _tasksTab() => _SearchList(
    ...
    matches: (item, text) =>
        '${item['source_label'] ?? ''} ${item['courier_name'] ?? ''} ${item['customer_name'] ?? ''} ${item['seller_name'] ?? ''} ${item['task_status'] ?? ''}'
            .toLowerCase()
            .contains(text),
    filters: const {
      'source_type': {
        'store_order': 'طلب متجر',
        'marketplace_transaction': 'صفقة حراج',
      },
      'task_status': {
        'available': 'متاحة',
        'accepted': 'مقبولة',
        'en_route_pickup': 'إلى الاستلام',
        'picked_up': 'تم الاستلام',
        'en_route_delivery': 'إلى التسليم',
    ...
    builder: (item) {
      final id = item['public_id'] as String? ?? '';
      final source = item['source_type'] == 'marketplace_transaction'
          ? 'حراج'
          : 'طلب متجر';
      return Card(
        child: ListTile(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            if (task['pickup_proof_media_public_id'] != null)
              OutlinedButton.icon(
                onPressed: () => _showProof(id, 'pickup'),
                icon: const Icon(Icons.inventory_2_outlined),
                label: const Text('إثبات الاستلام'),
              ),
            if (task['delivery_proof_media_public_id'] != null)
              OutlinedButton.icon(
                onPressed: () => _showProof(id, 'delivery'),
  ...
  Widget build(BuildContext context) =>
      FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final byFilters = all
              .where(
                (item) => _filterValues.entries.every(
                  (filter) =>
                          ...
                          (filter) => SizedBox(
                            ...
                            child: DropdownButtonFormField<String>(
                              ...
                              decoration: InputDecoration(
                                labelText: filter.key == 'source_type'
                                    ? 'مصدر المهمة'
                                    : filter.key == 'task_status'
                                    ? 'حالة المهمة'
                                    : filter.key == 'work_status'
                                    ? 'حالة العمل'
                                    : 'التوثيق',
                              ),
                              items: [
              ...
              Expanded(
                child: items.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (_, index) => widget.builder(items[index]),
...
Widget _taskListSection(String title, List<Map<String, dynamic>> tasks) =>
    Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 16),
        ...
        ...tasks.map(
          (task) => Card(
            child: ListTile(
              title: Text(task['source_label'] as String? ?? 'مهمة'),
              subtitle: Text(
                '${task['task_status'] ?? '—'} · ${task['created_at'] ?? ''}',
              ),
            ),
          ),
        ),
...
Widget _historySection(List<Map<String, dynamic>> history) => Column(
  crossAxisAlignment: CrossAxisAlignment.start,
  children: [
    const SizedBox(height: 16),
    const Text(
      'سجل تغيّر حالة المهمة',
