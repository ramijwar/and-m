...
final class _AdminCommercePageState extends State<AdminCommercePage> {
  ...
  Future<void> _editStore(Map<String, dynamic> store) async {
    ...
    try {
      ...
      final fullStore = raw is Map ? Map<String, dynamic>.from(raw) : store;
      if (!mounted) return;
      final saved = await Navigator.of(context).push<bool>(
        MaterialPageRoute<bool>(
          builder: (_) => StoreSettingsPage(store: fullStore, adminStoreId: id),
        ),
      );
  ...
  Future<void> _deleteStore(Map<String, dynamic> store) async {
    ...
    if (await _confirm(
      'حذف المتجر',
      'سيختفي المتجر ومنتجاته عن العملاء، مع الاحتفاظ بسجلات الطلبات والتدقيق.',
    )) {
      await _run(
        'store-$id',
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'السبب الذي سيحفظ في سجل الطلب',
          ),
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) => DefaultTabController(
    ...
    child: AdminWorkspaceScaffold(
      active: AdminDestination.commerce,
      appBar: AppBar(
      ...
      body: TabBarView(
        children: [_storesTab(), _productsTab(), _ordersTab(), _paymentsTab()],
      ),
    ),
  );
  ...
  Widget _storesTab() => _AdminList(
    ...
    itemBuilder: (item) {
      ...
      return _AdminEntityCard(
        ...
        loadDetails: id.isEmpty
            ? null
            : () async {
                final data = await AppScope.of(context)
                    .loadAdminStoreDetail(id);
                return {
                  ...Map<String, dynamic>.from(
                    data['store'] as Map? ?? const {},
                  ),
                  'store_products': data['products'] ?? const [],
                  'store_orders': data['orders'] ?? const [],
                  'store_reviews': data['reviews'] ?? const [],
                  'latest_verification': data['latest_verification'],
                };
              },
  ...
  Widget _productsTab() => _AdminList(
    ...
    itemBuilder: (item) {
      ...
      return _AdminEntityCard(
        ...
        summary:
            '${item['store_name'] ?? 'متجر'} · ${item['price'] ?? 0} ${item['currency_code'] ?? ''}\nالمخزون: $stock · الحالة: ${item['product_status'] ?? '—'}',
        item: item,
        detailTitle: 'تفاصيل المنتج',
        loadDetails: id.isEmpty
            ? null
            : () => AppScope.of(context).loadAdminProductDetail(id),
            ...
            : [
                _Action(
                  ...
                  () => _run(
                    ...
                    () => AppScope.of(
                      context,
                    ).updateAdminProductStatus(productId: id, status: 'active'),
                    'تم نشر المنتج.',
                  ),
                ),
                _Action(
                  ...
                  () => _run(
                    ...
                    () => AppScope.of(
                      context,
                    ).updateAdminProductStatus(productId: id, status: 'hidden'),
                    'تم إخفاء المنتج.',
                  ),
                ),
                _Action(
                  ...
                  () => _run(
                    'product-$id',
                    () => AppScope.of(context).updateAdminProductStatus(
                      productId: id,
                      status: 'archived',
                    ),
                    'تمت أرشفة المنتج.',
                ...
                _Action('حذف منطقي', Icons.delete_outline, () async {
                  if (await _confirm(
                    'حذف المنتج من العرض',
                    'سيُحذف المنتج من العرض فقط ويحافظ الخادم على السجلات المالية السابقة.',
                  )) {
                    _run(
                      'product-$id',
                      () => AppScope.of(context).adminDeleteProduct(id),
  ...
  Widget _ordersTab() => _AdminList(
    ...
    filterOptions: const {
      ...
      'payment_verified': 'دفع معتمد',
      'merchant_accepted': 'مقبول',
      'preparing': 'تحضير',
      'ready_for_delivery': 'جاهز للتوصيل',
      'assigned_to_courier': 'مسند لعامل',
    ...
    itemBuilder: (item) {
      ...
      return _AdminEntityCard(
        ...
        loadDetails: id.isEmpty
            ? null
            : () => AppScope.of(context).loadAdminOrderDetail(id),
...
final class _AdminListState extends State<_AdminList> {
  ...
  Widget build(
    ...
    builder: (context, snapshot) {
      ...
      final filtered = widget.filterField == null || _selectedFilter.isEmpty
          ? all
          : all
                .where(
                  (item) =>
                      '${item[widget.filterField] ?? ''}' == _selectedFilter,
                )
        ...
        children: [
            ...
            Padding(
              ...
              child: DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  labelText: widget.filterLabel ?? 'تصفية',
                ),
                items: [
          ...
          Expanded(
            child: items.isEmpty
                ? Center(
                    child: Text(
                      query.isEmpty ? widget.empty : 'لا توجد نتائج مطابقة.',
                    ),
                  )
                : ListView.separated(
  ...
  try {
    ...
    if (context.mounted)
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تعذر تحميل التفاصيل الكاملة.')),
      );
  }
}
...
void _showDetails(
  ...
  const labels = {
    ...
    'sale_starts_at': 'بداية التخفيض',
    'sale_ends_at': 'نهاية التخفيض',
    'currency_code': 'العملة',
    'stock_quantity': 'المخزون',
    'low_stock_threshold': 'حد التنبيه',
    'product_status': 'حالة المنتج',
    'store_name': 'المتجر',
    'store_public_id': 'معرف المتجر',
    'store_status': 'حالة المتجر',
    'merchant_name': 'التاجر',
    'merchant_public_id': 'معرف التاجر',
    'category_name': 'التصنيف',
    'category_public_id': 'معرف التصنيف',
    'view_count': 'المشاهدات',
    'order_number': 'رقم الطلب',
    'order_status': 'حالة الطلب',
    'payment_status': 'حالة الدفع',
    'payment_method': 'طريقة الدفع',
    'fulfillment_type': 'نوع الاستلام',
    'customer_name': 'العميل',
    'customer_public_id': 'معرف العميل',
    'customer_phone': 'هاتف العميل',
    'customer_note': 'ملاحظة العميل',
    'subtotal_amount': 'الإجمالي الفرعي',
    'delivery_fee_amount': 'رسوم التوصيل',
    'platform_fee_amount': 'رسوم المنصة',
    'total_amount': 'الإجمالي',
    'item_count': 'عدد العناصر',
    'placed_at': 'وقت الطلب',
    'invoice_number': 'رقم الفاتورة',
    'enabled': 'التحويل اليدوي مفعّل',
    'deposit_title': 'عنوان الإيداع',
    'deposit_address': 'تفاصيل الإيداع',
    'instructions': 'تعليمات العميل',
    'payment_public_id': 'معرف إشعار الدفع',
    'order_public_id': 'معرف الطلب',
    'payer_note': 'إشعار العميل الكتابي',
    'transfer_reference': 'مرجع التحويل',
    'amount': 'مبلغ الإشعار',
    'has_receipt': 'يتضمن صورة إيصال',
    'submitted_at': 'وقت الإرسال',
    'merchant_reviewer_name': 'التاجر المراجع',
    'merchant_reviewed_at': 'وقت قرار التاجر',
    'merchant_note': 'ملاحظة التاجر',
    'order_payment_status': 'حالة دفع الطلب',
  ...
  final media = (item['media'] as List? ?? const [])
      ...
      .toList(growable: false);
  final lines = (item['items'] as List? ?? const [])
      .whereType<Map>()
      .map((value) => Map<String, dynamic>.from(value))
      .toList(growable: false);
  final history = (item['history'] as List? ?? const [])
      ...
      .map((value) => Map<String, dynamic>.from(value))
      .toList(growable: false);
  final storeProducts = (item['store_products'] as List? ?? const [])
      .whereType<Map>()
      .map((value) => Map<String, dynamic>.from(value))
      .toList(growable: false);
  final storeOrders = (item['store_orders'] as List? ?? const [])
      .whereType<Map>()
      .map((value) => Map<String, dynamic>.from(value))
      .toList(growable: false);
  final storeReviews = (item['store_reviews'] as List? ?? const [])
      .whereType<Map>()
      .map((value) => Map<String, dynamic>.from(value))
      .toList(growable: false);
  final verification = item['latest_verification'] is Map
      ? Map<String, dynamic>.from(item['latest_verification'] as Map)
      : null;
  final basicRows = item.entries
      .where(
        (entry) =>
            entry.value != null &&
            entry.value is! List &&
          ...
          children: [
            ...
            if (media.isNotEmpty) ...[
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: media
                    .where((entry) => entry['public_id'] is String)
                    .map(
                      (entry) => ClipRRect(
                        ...
                        child: SizedBox(
                          ...
                          child: CachedMediaImage(
                            mediaPublicId: entry['public_id'] as String,
                          ),
                        ),
                      ),
                    )
            ...
            if (lines.isNotEmpty) ...[
              ...
              ...lines.map(
                (line) => Card(
                  child: ListTile(
                    title: Text(
                      line['product_name_snapshot'] as String? ?? 'منتج',
                    ),
                    subtitle: Text(
                      '${line['quantity'] ?? 0} × ${line['unit_price_amount'] ?? 0} · ${line['sku_snapshot'] ?? 'دون SKU'}',
                    ),
                    trailing: Text('${line['line_total_amount'] ?? 0}'),
                  ),
                ),
              ),
            ],
            mapBlock(
              'إعدادات التحويل اليدوي الحالية',
              item['manual_payment'] is Map
                  ? Map<String, dynamic>.from(item['manual_payment'] as Map)
                  : null,
            ),
            mapBlock(
              'لقطة المتجر وقت الطلب',
              item['store_snapshot'] is Map
                  ? Map<String, dynamic>.from(item['store_snapshot'] as Map)
                  : null,
            ),
            mapBlock(
              'تعليمات الإيداع وقت الطلب',
              item['manual_payment_snapshot'] is Map
                  ? Map<String, dynamic>.from(
                      item['manual_payment_snapshot'] as Map,
                    )
                  : item['deposit_snapshot'] is Map
                  ? Map<String, dynamic>.from(item['deposit_snapshot'] as Map)
                  : null,
            ),
            mapBlock(
              'تفاصيل إشعار الدفع',
            ...
            mapBlock(
              'عنوان التسليم وقت الطلب',
              item['delivery_address'] is Map
                  ? Map<String, dynamic>.from(item['delivery_address'] as Map)
                  : null,
            ),
            ...
            if (storeProducts.isNotEmpty) ...[
              ...
              ...storeProducts.map(
                (row) => Card(
                  child: ListTile(
                    title: Text(row['name'] as String? ?? 'منتج'),
                    subtitle: Text(
                      '${row['price'] ?? 0} ${row['currency_code'] ?? ''} · مخزون ${row['stock_quantity'] ?? 0} · ${row['product_status'] ?? '—'}',
                    ),
                  ),
                ),
              ),
            ...
            if (storeOrders.isNotEmpty) ...[
              ...
              ...storeOrders.map(
                (row) => Card(
                  child: ListTile(
                    title: Text(row['order_number'] as String? ?? 'طلب'),
                    subtitle: Text(
            ...
            if (storeReviews.isNotEmpty) ...[
              ...
              ...storeReviews.map(
                (row) => Card(
                  child: ListTile(
                    title: Text(
                      '${row['customer_name'] ?? 'عميل'} · ${row['rating'] ?? 0}/5',
                    ),
                    subtitle: Text(row['comment'] as String? ?? 'دون تعليق'),
                  ),
                ),
              ),
            ],
            if (history.isNotEmpty) ...[
              ...
              ...history.map(
                (entry) => Card(
                  child: ListTile(
                    title: Text(
                      '${entry['from_status'] ?? 'بداية'} ← ${entry['to_status'] ?? '—'}',
                    ),
                    subtitle: Text(
                      '${entry['actor_name'] ?? 'النظام'} · ${entry['created_at'] ?? ''}\n${entry['note'] ?? ''}',
                    ),
                  ),
                ),
              ),
            ...
            if ((item['payment_history'] as List? ?? const []).isNotEmpty) ...[
              ...
              ...((item['payment_history'] as List? ?? const []).whereType<Map>().map(
                (entry) => Card(
                  child: ListTile(
                    title: Text(
                      '${entry['from_status'] ?? 'بداية'} ← ${entry['to_status'] ?? '—'}',
