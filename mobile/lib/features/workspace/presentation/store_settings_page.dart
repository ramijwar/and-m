...
final class StoreSettingsPage extends StatefulWidget {
  const StoreSettingsPage({super.key, required this.store, this.adminStoreId});
  final Map<String, dynamic> store;
  /// When present the screen preserves all validation but commits as an administrator.
...
final class _StoreSettingsPageState extends State<StoreSettingsPage> {
  ...
  Future<void> _save() async {
    ...
    try {
      final values = <String, dynamic>{
        ...
        if (widget.adminStoreId == null) 'store_enabled': _activate,
      };
      if (widget.adminStoreId == null) {
        await AppScope.of(context).updateMerchantStore(logo: _logo, values: values);
      } else {
        await AppScope.of(context).updateAdminStore(
          storeId: widget.adminStoreId!, logo: _logo, values: values,
        );
      }
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(widget.adminStoreId == null ? 'إعدادات المتجر' : 'إدارة بيانات المتجر'),
      actions: widget.adminStoreId == null
          ? [IconButton(tooltip: 'إعلانات متجري', icon: const Icon(Icons.campaign_outlined), onPressed: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const StoreBannerManagementPage(admin: false))))]
          : null,
    ),
