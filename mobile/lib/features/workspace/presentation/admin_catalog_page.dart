...
final class AdminCatalogPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DefaultTabController(
    ...
    child: AdminWorkspaceScaffold(
      active: AdminDestination.catalog,
      appBar: AppBar(
...
final class _CatalogCategoryDialogState extends State<_CatalogCategoryDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_name.text.trim().length < 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('اسم التصنيف يجب أن يحتوي حرفين على الأقل.'),
        ),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final parents = widget.siblings
        .where((item) => item['public_id'] != ownId)
        .toList(growable: false);
    return AlertDialog(
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text(
                  _image == null
                      ? 'إرفاق صورة التصنيف (اختياري)'
                      : 'تم اختيار صورة التصنيف — تغييرها',
                ),
              ),
              SwitchListTile.adaptive(
                ...
                subtitle: const Text(
                  'إيقاف التصنيف يحافظ على بياناته وعناصره السابقة.',
                ),
              ),
            ],
          ),
