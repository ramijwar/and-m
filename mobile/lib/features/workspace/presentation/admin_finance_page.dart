...
final class _AdminFinancePageState extends State<AdminFinancePage>
  ...
  void _reload() {
    ...
    setState(() {
      ...
      _withdrawals = api.loadAdminDecisionQueues().then(
  ...
  List<Map<String, dynamic>> _filter(List<Map<String, dynamic>> source) {
    ...
    return source
        .where(
          (item) =>
              (_currency.isEmpty || item['currency_code'] == _currency) &&
              (term.isEmpty ||
                  item.values.any((v) => '$v'.toLowerCase().contains(term))),
        )
  ...
  Widget build(BuildContext context) => AdminWorkspaceScaffold(
    active: AdminDestination.finance,
    appBar: AppBar(
      ...
      children: [
        FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final wallets = (values['wallets'] as List? ?? const [])
                .whereType<Map>()
                .toList();
            final entries = (values['entries'] as List? ?? const [])
                .whereType<Map>()
                .toList();
            final invoices = (values['invoices'] as List? ?? const [])
                .whereType<Map>()
                .toList();
            return SizedBox(
              height: 78,
                ...
                children: [
                  _metric(
                    ...
                    wallets.fold<int>(
                      0,
                      (n, row) =>
                          n + ((row['wallet_count'] as num?)?.toInt() ?? 0),
                    ),
                  ...
                  _metric(
                    ...
                    entries.fold<int>(
                      0,
                      (n, row) =>
                          n + ((row['entry_count'] as num?)?.toInt() ?? 0),
                    ),
                  ...
                  _metric(
                    ...
                    invoices.fold<int>(
                      0,
                      (n, row) =>
                          n + ((row['invoice_count'] as num?)?.toInt() ?? 0),
                    ),
              ...
              Expanded(
                child: TextField(
                  decoration: const InputDecoration(
                    ...
                    hintText: 'بحث بالاسم أو المرجع أو المعرف',
  ...
  Widget _walletsTab() => FutureBuilder<List<Map<String, dynamic>>>(
    future: _wallets,
    builder: (context, snapshot) {
      if (snapshot.connectionState != ConnectionState.done)
        return const Center(child: CircularProgressIndicator());
      if (snapshot.hasError)
        return const Center(child: Text('تعذر تحميل المحافظ.'));
      final rows = _filter(snapshot.data ?? const []);
      if (rows.isEmpty)
        return const Center(child: Text('لا توجد محافظ مطابقة.'));
            ...
            child: ListTile(
              ...
              title: Text(
                row['owner_name'] as String? ??
                    (row['owner_type'] == 'platform' ? 'محفظة المنصة' : key),
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
              subtitle: Text(
                'متاح: ${row['available_amount'] ?? 0} $currency · محجوز: ${row['held_amount'] ?? 0}\n${row['owner_type'] ?? '—'} · $key',
              ),
              isThreeLine: true,
              trailing: const Icon(Icons.chevron_left_rounded),
              onTap: key.isEmpty || currency.isEmpty
                  ? null
                  : () => _showWallet(key, currency),
            ),
          );
        },
      );
  ...
  Widget _entriesTab() => FutureBuilder<List<Map<String, dynamic>>>(
    future: _entries,
    builder: (context, snapshot) {
      if (snapshot.connectionState != ConnectionState.done)
        return const Center(child: CircularProgressIndicator());
      if (snapshot.hasError)
        return const Center(child: Text('تعذر تحميل القيود المالية.'));
      final rows = _filter(snapshot.data ?? const []);
      if (rows.isEmpty)
        return const Center(child: Text('لا توجد قيود مطابقة.'));
      return ListView.separated(
        padding: const EdgeInsets.fromLTRB(16, 12, 16, 28),
        itemCount: rows.length,
        separatorBuilder: (_, _) => const SizedBox(height: 8),
        itemBuilder: (_, i) {
          final row = rows[i];
          final plus = row['direction'] == 'credit';
          return Card(
            child: ListTile(
              ...
              title: Text(
                '${plus ? '+' : '−'}${row['amount'] ?? 0} ${row['currency_code'] ?? ''}',
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
              subtitle: Text(
                '${row['entry_type'] ?? 'قيد'} · ${row['owner_name'] ?? row['account_key'] ?? ''}\n${row['reference_type'] ?? ''}: ${row['reference_id'] ?? ''}\n${row['note'] ?? row['created_at'] ?? ''}',
              ),
              isThreeLine: true,
            ),
          );
  ...
  Widget _invoicesTab() => FutureBuilder<List<Map<String, dynamic>>>(
    future: _invoices,
    builder: (context, snapshot) {
      if (snapshot.connectionState != ConnectionState.done)
        return const Center(child: CircularProgressIndicator());
      if (snapshot.hasError)
        return const Center(child: Text('تعذر تحميل الفواتير.'));
      final rows = _filter(snapshot.data ?? const []);
      if (rows.isEmpty)
        ...
        itemBuilder: (_, i) {
          ...
          return Card(
            child: ListTile(
              ...
              title: Text(
                row['invoice_number'] as String? ?? 'فاتورة',
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
              subtitle: Text(
                '${row['store_name'] ?? 'متجر'} · ${row['customer_name'] ?? 'عميل'}\nالإجمالي: ${row['total_amount'] ?? 0} ${row['currency_code'] ?? ''} · ${row['order_status'] ?? '—'}',
              ),
              isThreeLine: true,
              onTap: () => _showRows('تفاصيل الفاتورة', row),
            ),
          );
        },
      );
  ...
  Widget _withdrawalsTab() => FutureBuilder<List<Map<String, dynamic>>>(
    future: _withdrawals,
    builder: (context, snapshot) {
      if (snapshot.connectionState != ConnectionState.done)
        return const Center(child: CircularProgressIndicator());
      if (snapshot.hasError)
        return const Center(child: Text('تعذر تحميل طلبات السحب.'));
      final rows = _filter(snapshot.data ?? const []);
      if (rows.isEmpty)
        return const Center(child: Text('لا توجد طلبات سحب مطابقة.'));
      return ListView.separated(
        padding: const EdgeInsets.fromLTRB(16, 12, 16, 28),
        itemCount: rows.length,
        separatorBuilder: (_, _) => const SizedBox(height: 8),
        itemBuilder: (_, i) {
          ...
          return Card(
            child: ListTile(
              ...
              title: Text(
                '${row['amount'] ?? 0} ${row['currency_code'] ?? ''}',
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
              subtitle: Text(
                '${row['request_status'] ?? '—'} · ${row['created_at'] ?? ''}\n${row['decision_note'] ?? 'افتح طابور الإدارة لاتخاذ القرار.'}',
              ),
              isThreeLine: true,
              trailing: row['request_status'] == 'pending'
                  ? TextButton(
                      onPressed: () => openAdminDestination(
                        context,
                        AdminDestination.queues,
                      ),
  ...
  Future<void> _showWallet(String key, String currency) async {
    try {
      final data = await AppScope.of(context)
          .loadAdminFinancialWalletDetail(key, currency);
      if (!mounted) return;
                ...
                if (withdrawals.isNotEmpty) ...[
                  ...
                  ...withdrawals.map(
                    (r) => Card(
                      child: ListTile(
                        title: Text(
                          '${r['amount']} ${r['currency_code']} · ${r['request_status']}',
                        ),
                        subtitle: Text(
                          '${r['decision_note'] ?? ''}\n${r['created_at'] ?? ''}',
                        ),
                ...
                if (entries.isNotEmpty) ...[
                  ...
                  ...entries.map(
                    (r) => Card(
                      child: ListTile(
                        title: Text(
                          '${r['direction'] == 'credit' ? '+' : '−'}${r['amount']} · ${r['entry_type']}',
                        ),
                        subtitle: Text(
  ...
  void _showRows(String title, Map<String, dynamic> values) =>
  ...
  Widget _rows(String title, Map<String, dynamic> values) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
