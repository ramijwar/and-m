...
final class _MerchantWorkspacePageState extends State<MerchantWorkspacePage> {
  ...
  void _reload() =>
      setState(() => _future = AppScope.of(context).loadMerchantWorkspace());
          ...
          return RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                Text(
                  closed ? 'تم إغلاق المتجر' : 'ابدأ بإنشاء متجرك',
                ...
                Text(
                  closed
                      ? 'أُغلق متجر هذا الحساب من الإدارة، ولا يمكن إنشاء متجر بديل تلقائياً. افتح تذكرة دعم إذا احتجت مراجعة القرار.'
                      : 'لا يوجد متجر مرتبط بهذا الحساب بعد. أنشئ المتجر أولاً، ثم أكمل بياناته وموقعه وساعات العمل قبل تفعيله.',
                  textAlign: TextAlign.center,
          ...
          child: ListView(
            ...
            children: [
                ...
                onOpenSettings: () async {
                  final result = await Navigator.of(context).push<bool>(
                    MaterialPageRoute<bool>(
                      builder: (_) => StoreSettingsPage(store: store),
                    ),
                  );
              ...
              FilledButton.icon(
                ...
                label: Text('طلبات المتجر (${workspace.orders.length})'),
              ),
              const SizedBox(height: 12),
              if ((store['manual_payment'] as Map?)?['enabled'] == true)
                FilledButton.icon(
                  onPressed: () async {
                    await Navigator.of(context).push(
                      MaterialPageRoute<void>(
                        builder: (_) => const MerchantPaymentReviewPage(),
                      ),
                    );
                    if (mounted) _reload();
                  },
                  ...
                  label: Text(
                    'إشعارات دفع بانتظار المراجعة (${workspace.pendingPayments.length})',
                  ),
                )
              else
                const Card(
                  child: ListTile(
                    ...
                    title: Text('التحويل اليدوي غير مفعّل'),
                    subtitle: Text(
                      'فعّله وأدخل عنوان الإيداع من إعدادات المتجر.',
                    ),
                  ),
                ),
              const SizedBox(height: 12),
              if (store['store_status'] == 'active')
                FilledButton.icon(
                  onPressed: () => Navigator.of(context).push(
                    MaterialPageRoute<void>(
                      builder: (_) => const MerchantNotificationCampaignPage(),
                    ),
                  ),
                ...
                const Card(
                  child: ListTile(
                    ...
                    title: Text(
                      'فعّل المتجر أولاً لطلب حملة للمشتركين والعملاء.',
                    ),
                  ),
                ),
            ],
...
final class _StoreStatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (store == null)
      return const Card(
        child: Padding(
          ...
          child: Text('لم يتم العثور على متجر هذا الحساب.'),
        ),
      );
    ...
    return Card(
      color: active
          ? Theme.of(context).colorScheme.primaryContainer.withOpacity(.42)
          : null,
      child: Padding(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    active
                        ? 'متجرك نشط ويظهر للعملاء.'
                        : 'متجرك في حالة مسودة؛ أكمل الإعدادات ثم فعّله.',
                  ),
                ],
              ),
            ),
