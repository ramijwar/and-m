      ...
      builder: (context) => SafeArea(
        child: DraggableScrollableSheet(
          ...
          builder: (_, controller) => ListView(
            ...
            children: [
              Text(
                item['title'] as String? ?? 'تفاصيل الإعلان',
                ...
                return RefreshIndicator(
                  ...
                  child: ListView(
                    children: const [
                      SizedBox(
                        height: 240,
                        child: Center(child: Text('لا توجد إعلانات مطابقة.')),
                      ),
                    ],
                  ),
                );
...
final class _ListingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (status == 'pending_review') {
      actions.add(
        FilledButton(
          onPressed: busy
              ? null
              : () => onStatus('active', 'تم اعتماد الإعلان.'),
      ...
      actions.add(
        TextButton(
          onPressed: busy
              ? null
              : () => onStatus('rejected', 'تم رفض الإعلان.'),
      ...
      actions.add(
        TextButton(
          onPressed: busy
              ? null
              : () => onStatus('paused', 'تم إيقاف الإعلان.'),
      ...
      actions.add(
        FilledButton.icon(
          onPressed: busy
              ? null
              : () => onStatus('active', 'تم تشغيل الإعلان.'),
    ...
    actions.add(
      TextButton(onPressed: busy ? null : onDelete, child: const Text('حذف')),
    );
    return Card(
      child: Padding(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        item['title'] as String? ?? 'إعلان حراج',
                        maxLines: 2,
                      ...
                      Text(
                        '${item['seller_name'] ?? 'بائع'} · ${item['category_name'] ?? 'بدون قسم'}',
                      ),
                      Text(
                        '${item['price_amount'] ?? '—'} ${item['currency_code'] ?? ''} · ${item['city'] ?? ''}',
...
final class _AdminMarketplaceListingEditorPageState
  ...
  void initState() {
    ...
    _address = TextEditingController(
      text: item['address_text'] as String? ?? '',
    );
    ...
    _fulfillment =
        const [
          ...
          'delivery',
          'both',
        ].contains(item['fulfillment_method'])
    ...
    try {
      await AppScope.of(context).updateAdminMarketplaceListing(
        ...
        values: {
          ...
          'price_amount': _price.text.trim().isEmpty
              ? null
              : _price.text.trim(),
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Form(
      ...
      child: ListView(
        ...
        children: [
          const Text(
            'تُسجّل التعديلات باسم المدير، ولا تغيّر ملكية الإعلان أو سجل صفقتِه.',
          ),
          ...
          TextFormField(
            ...
            validator: (value) => (value?.trim().length ?? 0) < 4
                ? 'أدخل عنواناً من 4 أحرف على الأقل.'
                : null,
          ),
          const SizedBox(height: 8),
          TextFormField(
            ...
            validator: (value) =>
                (value?.trim().length ?? 0) < 10 ? 'أدخل وصفاً واضحاً.' : null,
          ),
          const SizedBox(height: 8),
          Row(
            children: [
          ...
          DropdownButtonFormField<String>(
            ...
            items: const [
              ...
              DropdownMenuItem(value: 'refurbished', child: Text('مجدّد')),
              DropdownMenuItem(
          ...
          DropdownButtonFormField<String>(
            ...
            items: const [
              DropdownMenuItem(value: 'pickup', child: Text('استلام مباشر')),
              DropdownMenuItem(value: 'delivery', child: Text('توصيل')),
              DropdownMenuItem(value: 'both', child: Text('توصيل أو استلام')),
            ],
            onChanged: (value) =>
                setState(() => _fulfillment = value ?? _fulfillment),
          ),
