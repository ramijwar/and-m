...
final class MerchantWorkspace {
  ...
  /// Active and historical store orders with actions authorized by the server.
...
final class WorkspaceRepository {
  ...
  Future<MerchantWorkspace> merchantWorkspace() async {
    ...
    if (store == null || (store['public_id'] as String? ?? '').isEmpty) {
      ...
      return MerchantWorkspace(
        ...
        pendingPayments: const <Map<String, dynamic>>[],
    ...
    final responses = await Future.wait([
      ...
      _api.get('/merchant/orders', query: const {'per_page': 50}),
    ]);
    final productRaw = responses[0]['items'];
    final paymentRaw = responses[1]['items'];
    final ordersRaw = responses[2]['items'];
    return MerchantWorkspace(
      store: store,
      ...
      pendingPayments: (paymentRaw is List ? paymentRaw : const [])
          .whereType<Map>()
          .map((item) => Map<String, dynamic>.from(item))
          .toList(growable: false),
      orders: (ordersRaw is List ? ordersRaw : const [])
          .whereType<Map>()
          .map((item) => Map<String, dynamic>.from(item))
          .toList(growable: false),
    );
  ...
  Future<List<Map<String, dynamic>>> adminCatalogCategories(
    String type,
  ) async => _mapItems(
    await _api.get('/admin/catalog-categories', query: {'type': type}),
  );
  ...
  Future<Uint8List> merchantPaymentReceipt(String orderId) => _api.getBytes(
    '/merchant/orders/${Uri.encodeComponent(orderId)}/payment-receipt',
  );

  Future<List<Map<String, dynamic>>> merchantOrders({String status = ''}) =>
      _mapItemsAsync(
        _api.get(
          '/merchant/orders',
          query: {'per_page': 50, if (status.isNotEmpty) 'status': status},
        ),
      );

  Future<void> transitionMerchantOrder({
    required String orderId,
    required String action,
  }) => _api.post(
    '/merchant/orders/${Uri.encodeComponent(orderId)}/transition',
    body: {'action': action},
    idempotencyKey: _operationKey('merchant-order-$action'),
  );

  Future<void> cancelMerchantOrder({
    required String orderId,
    required String reason,
  }) => _api.post(
    '/merchant/orders/${Uri.encodeComponent(orderId)}/cancel',
    body: {'reason': reason.trim()},
    idempotencyKey: _operationKey('merchant-order-cancel'),
  );

  Future<List<Map<String, dynamic>>> adminManualPaymentLog() => _mapItemsAsync(
    _api.get('/admin/payments/manual-log', query: {'per_page': '100'}),
  );
  ...
  Future<List<Map<String, dynamic>>> adminSupportTickets() =>
      _mapItemsAsync(_api.get('/admin/support/tickets'));

  Future<Map<String, dynamic>> adminSupportTicket(String ticketId) async {
    final data = await _api.get(
      '/admin/support/tickets/${Uri.encodeComponent(ticketId)}',
    );
    final raw = data['ticket'];
    return raw is Map ? Map<String, dynamic>.from(raw) : data;
  }
  ...
  Future<Map<String, dynamic>> adminStoreDetail(String storeId) =>
      _api.get('/admin/stores/${Uri.encodeComponent(storeId)}');

  Future<Map<String, dynamic>> adminProductDetail(String productId) async {
    final data = await _api.get(
      '/admin/products/${Uri.encodeComponent(productId)}',
    );
    return {
  ...
  Future<Map<String, dynamic>> adminOrderDetail(String orderId) async {
    final data = await _api.get(
      '/admin/orders/${Uri.encodeComponent(orderId)}',
    );
    return {
      ...
      if (data['store_snapshot'] is Map)
        'store_snapshot': Map<String, dynamic>.from(
          data['store_snapshot'] as Map,
        ),
      if (data['delivery_address'] is Map)
        'delivery_address': Map<String, dynamic>.from(
          data['delivery_address'] as Map,
        ),
    };
  }
  ...
  Future<List<Map<String, dynamic>>> adminMarketplaceListings({
    String status = '',
    String search = '',
  }) => _mapItemsAsync(
    _api.get(
  ...
  Future<void> updateAdminMarketplaceListing({
  ...
  Future<void> attachAdminMarketplaceListingMedia({
    required String listingId,
    required XFile image,
  }) async {
    final mediaId = await uploadAdminPublicMedia(image);
    await _api.post(
      '/admin/marketplace/listings/${Uri.encodeComponent(listingId)}/media',
  ...
  Future<List<Map<String, dynamic>>> merchantStoreBanners() async =>
  ...
  Future<void> saveStoreBanner({
    ...
    if (image != null)
      body['media_public_id'] = await _uploadPublicMedia(
        image,
        admin ? 'admin-store-banner-image' : 'merchant-store-banner-image',
      );
    final collection = admin
        ? '/admin/store-home-banners'
    ...
    if (bannerId == null || bannerId.isEmpty) {
      await _api.post(
        ...
        idempotencyKey: _operationKey(
          admin ? 'admin-store-banner-create' : 'merchant-store-banner-create',
        ),
      );
    } else {
      await _api.patch(
        '$collection/${Uri.encodeComponent(bannerId)}',
        body: body,
        idempotencyKey: _operationKey(
          admin ? 'admin-store-banner-update' : 'merchant-store-banner-update',
        ),
      );
    }
  }

  Future<void> deleteStoreBanner({
    required bool admin,
    required String bannerId,
  }) => _api.delete(
    '${admin ? '/admin/store-home-banners' : '/merchant/store/banners'}/${Uri.encodeComponent(bannerId)}',
    idempotencyKey: _operationKey(
      admin ? 'admin-store-banner-delete' : 'merchant-store-banner-delete',
    ),
  );
  ...
  Future<void> updateAdminStore({
    ...
    final body = Map<String, dynamic>.from(values);
    if (logo != null)
      body['logo_media_id'] = await uploadAdminPublicMedia(logo);
    await _api.patch(
      '/admin/stores/${Uri.encodeComponent(storeId)}',
      body: body,
    );
  }
          ...
          .map((item) {
            ...
            return <String, dynamic>{
              ...
              'delivery_address': snapshot(
                raw['delivery_address_snapshot'] ?? raw['delivery_address'],
              ),
            };
          })
          .toList(growable: false),
  ...
  Future<List<Map<String, dynamic>>> adminAuditLogs({String search = ''}) =>
      _mapItemsAsync(
        _api.get(
          '/admin/audit-logs',
          query: {
  ...
  Future<List<Map<String, dynamic>>> adminNotificationLogs({
    String search = '',
    String category = '',
  ...
  Future<List<Map<String, dynamic>>> merchantNotificationCampaignProducts({
    String search = '',
  }) => _mapItemsAsync(
    _api.get(
      '/merchant/notification-campaign-products',
      query: {
        'per_page': '12',
        if (search.trim().isNotEmpty) 'search': search.trim(),
      },
  ...
  Future<Map<String, dynamic>> createMerchantNotificationCampaign(
    Map<String, dynamic> values,
  ) => _api.post(
    '/merchant/notification-campaigns',
    body: values,
    idempotencyKey: _operationKey('merchant-notification-campaign'),
  );

  Future<List<Map<String, dynamic>>> adminMerchantNotificationCampaigns({
    String status = '',
  }) => _mapItemsAsync(
    _api.get(
      '/admin/merchant-notification-campaigns',
      query: {if (status.isNotEmpty) 'status': status},
    ),
  );

  Future<Map<String, dynamic>> reviewMerchantNotificationCampaign(
    String campaignId,
    Map<String, dynamic> values,
  ) => _api.post(
    '/admin/merchant-notification-campaigns/${Uri.encodeComponent(campaignId)}/review',
    body: values,
  );
  ...
  Future<List<Map<String, dynamic>>> adminNotificationCampaignTargets({
    ...
    _api.get(
      '/admin/notification-campaign-targets',
      query: {
        'type': type,
        'search': search.trim(),
        'per_page': '12',
  ...
  Future<Map<String, dynamic>> previewAdminNotificationCampaign(
    Map<String, dynamic> values,
  ) => _api.post('/admin/notification-campaigns/preview', body: values);

  Future<Map<String, dynamic>> createAdminNotificationCampaign(
    Map<String, dynamic> values,
  ) => _api.post(
    '/admin/notification-campaigns',
    body: values,
    idempotencyKey: _operationKey('admin-notification-campaign'),
  );

  Future<Map<String, dynamic>> adminFinancialSummary({
    String currencyCode = '',
  }) => _api.get(
  ...
  Future<Map<String, dynamic>> adminFinancialWalletDetail(
    String accountKey,
    String currencyCode,
  ) => _api.get(
  ...
  Future<List<Map<String, dynamic>>> adminFinancialInvoices({
    String search = '',
    String currencyCode = '',
  }) => _mapItemsAsync(
    _api.get(
  ...
  Future<void> acceptTask(String taskId, {String sourceType = 'store_order'}) =>
      _api.post(
  ...
  Future<List<Map<String, dynamic>>> _mapItemsAsync(
    Future<Map<String, dynamic>> data,
  ) async => _mapItems(await data);
