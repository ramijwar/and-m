...
final class _NotificationPreferencesPageState
  ...
  Widget build(BuildContext context) {
    final isAdmin =
        AppScope.of(context).session?.roles.contains('admin') == true;
    return Scaffold(
      ...
      body: FutureBuilder<List<NotificationPreference>>(
        ...
        builder: (context, snapshot) {
          ...
          _items ??= List<NotificationPreference>.from(
            snapshot.data ?? const [],
          );
          return ListView(
            ...
            children: [
              Card(
                child: Padding(
                  ...
                  child: Text(
                    isAdmin
                        ? 'يمكنك التحكم بإشعارات Push والتنبيهات غير الحرجة حسب الفئة. تبقى قرارات الإدارة الحرجة ظاهرة داخل التطبيق لجميع المديرين النشطين، حتى لا تضيع طلبات التوثيق والدفعات والسحب.'
                        : 'تحكم بما يظهر داخل التطبيق وما يصل عبر FCM لكل نوع من التنبيهات. تعطيل Push لا يلغي تسجيل جهازك، بل يوقف الإرسال لهذه الفئة فقط.',
                  ),
                ),
              ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SwitchListTile.adaptive(
                          ...
                          title: const Text('إظهار داخل التطبيق'),
                          value: item.inAppEnabled,
                          onChanged: _saving
                              ? null
                              : (value) => _update(entry.key, inApp: value),
                        ),
                        SwitchListTile.adaptive(
                          contentPadding: EdgeInsets.zero,
                          ...
                          onChanged: _saving
                              ? null
                              : (value) => _update(entry.key, push: value),
                        ),
                      ],
                    ),
                  ),
