...
final class _NotificationsPageState extends State<NotificationsPage> {
  ...
  void _reload() =>
      setState(() => _future = AppScope.of(context).loadNotifications());
  ...
  Future<void> _open(AppNotification notification) async {
    ...
    if (!opened && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'لا توجد وجهة مباشرة لهذا التنبيه؛ يمكنك مراجعة تفاصيله هنا.',
          ),
        ),
      );
    }
  ...
  Widget _pushStatus(BuildContext context) {
    ...
    final (icon, label, detail, color) = switch ((permission, registration)) {
      (PushPermissionState.granted, PushDeviceRegistrationState.registered) => (
        Icons.check_circle_outline_rounded,
        'إشعارات الجهاز مفعّلة',
        'تم تسجيل هذا الجهاز لاستقبال إشعارات تجارتي.',
        Theme.of(context).colorScheme.primary,
      ),
      (PushPermissionState.granted, PushDeviceRegistrationState.registering) =>
        (
          Icons.sync_rounded,
          'جارٍ تسجيل الجهاز',
          'يتم ربط الجهاز بخدمة الإشعارات.',
          Theme.of(context).colorScheme.primary,
        ),
      (PushPermissionState.granted, PushDeviceRegistrationState.failed) => (
        Icons.cloud_off_outlined,
        'تعذر تسجيل الجهاز',
        'تحقق من الإنترنت ثم حدّث هذه الصفحة لإعادة المحاولة.',
        Theme.of(context).colorScheme.error,
      ),
      (PushPermissionState.denied, _) => (
        Icons.notifications_off_outlined,
        'إذن الإشعارات موقوف',
        'فعّله من إعدادات Android لتصلك التنبيهات الخارجية.',
        Theme.of(context).colorScheme.error,
      ),
      (PushPermissionState.unavailable, _) => (
        Icons.info_outline_rounded,
        'إشعارات الجهاز غير متاحة',
        'يبقى سجل الإشعارات داخل التطبيق متاحاً.',
        Theme.of(context).colorScheme.onSurfaceVariant,
      ),
      _ => (
        Icons.notifications_paused_outlined,
        'إشعارات الجهاز بانتظار التفعيل',
        'فعّل الإذن لتسجيل هذا الجهاز واستلام التنبيهات.',
        Theme.of(context).colorScheme.onSurfaceVariant,
      ),
    };
    return Padding(
      ...
      child: Card(
        child: ListTile(
          ...
          trailing: IconButton(
            ...
            icon: const Icon(Icons.refresh_rounded),
            onPressed: () async {
              await controller.refreshPushPermissionState();
              if (mounted) setState(() {});
            },
          ),
        ),
